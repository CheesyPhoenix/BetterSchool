...
async function update() {
    ...
    let schools = JSON.parse(fs.readFileSync("./data/allSchools.json").toString());
    let _data = [];
    ...
    results.forEach((result) => {
        console.log(`Update for "${result.class}": ${result.success
            ? asciiGreen + "successfull!"
            : asciiRed + "failed!"} ${asciiReset} (${result.schoolName})`);
    });
    return _data;
...
function migrateAccounts() {
    const pass = JSON.parse(fs.readFileSync("./creds/pass.json").toString());
    let newPass = [];
    for (let i = 0; i < pass.length; i++) {
        ...
        if (!element.classID) {
            newPass.push({
                ...
                schoolID: uuidv5("https://amalieskram-vgs.inschool.visma.no/", uuidNamespace),
            });
        }
        else {
...
export { update, addToPass, migrateAccounts };
